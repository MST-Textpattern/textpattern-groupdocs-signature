...
$plugin['name'] = 'gdv_groupdocs_signature';
...
if (@txpinterface == 'admin') {
    ...
    register_callback('gdv_append_button_signature', 'article_ui', 'title');
}

// add GroupDocs button
function gdv_append_button_signature($event, $step, $data, $rs) {
    
    $js= gdv_javascript_signature();
    $button = '<input type="button" value="Insert GroupDocs Signature Form ID" onclick="gdv_insert_formid()">';
...
function gdv_javascript_signature(){
    ...
    $r= '<script>';
    $r.= 'function gdv_insert_formid(){
        
            // Enter GroupDocs Form ID
            ...
            var gdv_iframe_signature = \'<iframe src="https://apps.groupdocs.com/signature/forms/SignEmbed/\'+ans+\'?&referer=\'+cmsName+\'/\'+pluginVersion+\'" frameborder="0" width="600" height="400"></iframe>\';
            
            // insert in the end of <textarea id="body">
            var gdv_body_signature = $("textarea#body").html()+gdv_iframe_signature;
            $("textarea#body").html(gdv_body_signature);
    }';
    $r.= '</script>';
    return $r;
}
